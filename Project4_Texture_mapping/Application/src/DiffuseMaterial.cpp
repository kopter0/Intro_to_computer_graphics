...
void DiffuseMaterial::CreateMaterial(int unit)
{
	...
	location = glGetUniformLocation(_program->GetProgramId(), "TextureEnabled");
	glUniform1i(location, 1);
}

void DiffuseMaterial::UpdateOurTexture(int unit) {
	glUseProgram(_program->GetProgramId());
	GLuint location = glGetUniformLocation(_program->GetProgramId(), "ourTexture");
	glUniform1i(location, unit);
	location = glGetUniformLocation(_program->GetProgramId(), "TextureEnabled");
	glUniform1i(location, 1);
}

void DiffuseMaterial::DisableTexture() {
	glUseProgram(_program->GetProgramId());
	GLuint location = glGetUniformLocation(_program->GetProgramId(), "TextureEnabled");
...
void DiffuseMaterial::UpdateLight(std::vector<Light> &lights)
{
	...
	for (int i = 0; i < numLights; i++)
	{
		...
		snprintf(buf, BUF_SIZE, "lights[%d].LightProj", i);
		location = glGetUniformLocation(pid, buf);
		glUniformMatrix4fv(location, 1, GL_FALSE, (float*)&lights[i].LightProj);
	}
}

void DiffuseMaterial::BindShadowMap(int unit) {
	glUseProgram(_program->GetProgramId());
...
void DiffuseMaterial::LightProjection(glm::mat4 proj) {
	glUseProgram(_program->GetProgramId());
	GLuint location = glGetUniformLocation(_program->GetProgramId(), "LightProj");
	glUniformMatrix4fv(location, 1, GL_FALSE, (float*)&proj);
}